...
describe ChatopsDeployer::Container do
  ...
  describe '#build' do
    ...
    it 'creates a VM with docker machine' do
      ...
      expect(ChatopsDeployer::Command).to receive(:run)
        .with(command: "docker-machine ssh fake_sha1 'sudo mkdir -p /cache'", logger: container.logger)
      expect(ChatopsDeployer::Command).to receive(:run)
        .with(command: "docker-machine ssh fake_sha1 'sudo mount -t vboxsf -o defaults,uid=`id -u docker`,gid=`id -g docker` cache /cache'", logger: container.logger)
      expect(ChatopsDeployer::Command).to receive(:run)
